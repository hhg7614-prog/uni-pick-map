...
// introduced -- .pipeline/spec.md 질문사항 3, 결정됨).
//
...
// without going through Brain's review + signed approval. Unlike the other
// activate-*.js scripts it never called saveNewItems() (catalog-only writer),
// but its activation result (source "catholic-kwandong-general-feed",
// enabled: true / verified: true / status: "verified" / healthStatus:
// "validated") was already present in the catalog before this stub was
// written -- confirmed by direct 3-way comparison against merge-base/origin
// during the git-divergence merge round (.pipeline/merge-analysis.md,
// section 1(b)). It has been replaced by the review-packet -> signed
// ReviewDecision -> --apply flow:
//
...
function main() {
  console.error(
    "[DEPRECATED] server/agent/tools/activate-catholic-kwandong-source-local.js was invoked but is retired. " +
  ...
  throw new Error(
    "activate-catholic-kwandong-source-local.js is deprecated and no longer performs source activation. " +
