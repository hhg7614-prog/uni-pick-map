...
function parseArgs(argv) {
  const options = { dryRun: false, limit: 20, includeUnimportant: false };
  for (const arg of argv) {
    if (arg === "--dry-run") options.dryRun = true;
    if (arg === "--include-unimportant") options.includeUnimportant = true;
...
function previewItem(item) {
  const aiResult = item.aiProcessed && item.aiResult && typeof item.aiResult === "object" ? item.aiResult : null;
  return {
    id: item.id,
    ...
    categoryLabel: aiResult?.categoryLabel || item.categoryLabel || "",
    title: item.title,
    summary: aiResult?.summary || item.summary || "",
    keywords: Array.isArray(aiResult?.keywords) ? aiResult.keywords.slice(0, 3) : [],
    ...
    hasAiSummary: Boolean(aiResult),
    aiProvider: item.aiProvider || "",
    isMockAiResult: Boolean(aiResult && item.aiProvider === "mock")
  };
}
...
function main() {
  ...
  const eligible = store.items.filter((item) => {
    ...
    const hideUnimportant = item.aiProcessed && item.aiResult?.isImportant === false && Number(item.aiResult.confidence) >= 0.7 && !options.includeUnimportant;
    if (hideUnimportant) return false;
    return valid;
  ...
  if (!options.dryRun) {
    // Keep the last working preview intact if a write is interrupted.
    writeAtomically(OUTPUT_FILE, `${JSON.stringify(output, null, 2)}\n`);
    writeAtomically(REPORT_JSON, `${JSON.stringify(report, null, 2)}\n`);
    writeAtomically(REPORT_MD, buildMarkdown(report));
  }
